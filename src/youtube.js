const youtubeEndpoint = `https://proxy.wingram.workers.dev?proxyUrl=https://www.youtube.com`;
...
const GetPlaylistData = async (playlistId, limit = 0) => {
  ...
  try {
    ...
    if (sectionListRenderer && sectionListRenderer.contents) {
      const videoItems =
        await sectionListRenderer.contents.twoColumnBrowseResultsRenderer
...
const GetSuggestData = async (limit = 0) => {
  ...
  try {
    const page = await GetYoutubeInitData(endpoint);
    const sectionListRenderer =
      await page.initdata.contents.twoColumnBrowseResultsRenderer.tabs[0]
...
const GetVideoDetails = async (videoId) => {
  ...
  try {
    ...
    const firstContent =
      await result.results.results.contents[0].videoPrimaryInfoRenderer;
    const secondContent =
      await result.results.results.contents[1].videoSecondaryInfoRenderer;
    const res = await {
